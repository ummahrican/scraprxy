...
def scrape(method, verbose=False):
    return asyncio.run(scrape_async(method, verbose))


async def scrape_async(method, verbose=False):
    now = time.time()
    methods = [method]
    if method == "socks":
    ...
    #         loop.run_until_complete(scrape(args.proxy, args.verbose))
    #         loop.close()
    #     elif sys.version_info >= (3, 7):
    #         asyncio.run(scrape(args.proxy, args.verbose))
    #     else:
    #         loop = asyncio.get_event_loop()
    #         loop.run_until_complete(scrape(args.proxy, args.verbose))
    #         loop.close()
