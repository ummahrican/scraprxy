...
from scraprxy.user_agents import user_agents
import socks
...
def check(
    unchecked_proxies, method, site, timeout=20, verbose=False, random_user_agent=True
):
    proxies = []
    for p in unchecked_proxies:
        proxies.append(Proxy(method, p))
    ...
    return [str(vp) for vp in valid_proxies]
